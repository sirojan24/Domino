...
class User {
	...
	*/
	...
	function isFriendRequestSent($userid){
		if($this->db->getRow("SELECT `requestid` FROM `".MLS_PREFIX."friend_requests` WHERE `senderid`=".$this->data->userid." AND `receiverid`=".$userid." AND `status`=true"))
			return TRUE;
		return FALSE;
	}
	
	function isFriendRequestCome($userid){
		if($this->db->getRow("SELECT `requestid` FROM `".MLS_PREFIX."friend_requests` WHERE `receiverid`=".$this->data->userid." AND `senderid`=".$userid." AND `status`=true"))
			return TRUE;
		return FALSE;
	}
