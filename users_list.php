...
$where = 'WHERE `groupid`!=4 AND userid!='.$user->data->userid;	//filter admin and current user

if(isset($_GET['q'])) 
  $where = $db->parse("WHERE `groupid`!=4 AND userid!=".$user->data->userid." AND`username` LIKE ?s", '%'.$_GET['q'].'%');		//filter admin and current user
